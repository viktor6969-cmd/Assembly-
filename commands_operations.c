...
typedef struct commands{
    /*A structure of commands, witch gives to
    every command number, binary code, and the number
    of operands that can be used with this command*/
    char* name;
    char* binary_code;
    int operands;
}commands;
...
/*----------GLOBAL VARIABLES------------*/
char* red_error_str = "\x1b[31m ERROR\033[0m";/*Red error message*/
commands command_list[COMMAND_LIST_SIZE];/*The command list, realized as an arrey of known size, we only have 16 commands*/
char binary_line[MAX_BINARY_LINE_SIZE];/*14 bit line*/
char* file_name_global;
label* temp;
int comm_num = -1;/*The number of the curent command, setet to -1 by defoult*/
int origin_rows;/*The rows of the original file*/

/*------------DECLARATIONS--------------*/
int operands_use_valid(int comm_num,int type1, int type2);/*Check that the operands used properly*/
int write_operand(int type,int IC, char* first);/*Writes the operand in binary code to the output list*/
int command_line_check(char* line);/*Check validation of command line*/
int command_number(char* name);/*Define the number of a command*/
int opearnd_type(char* line);/*Finds the operand type*/

/*--------------Functions---------------*/
int command_sort(char* name,char* line,int IC,int rows,char* file_name){

    /*gets as input the name of a command, the rest of the line, IC, rows number, and original file name
    finds the command name in progrram list, if founded sorts the operands and writes them.
    If there is any errors, return the number of them as negative number
    if the commands was writen, returns the number of rows as positive number*/

    char *first_operand = (char*)calloc(MAX_LINE_SIZE, sizeof(char));
    char *secnd_operand = (char*)calloc(MAX_LINE_SIZE, sizeof(char));
    int type1;/*Operand sort type*/
    int type2;
    int temp;
    ...
    *secnd_operand = '\0';
    ...
    switch(command_line_check(line)){
        ...
        case 1:
            ...
            /*If not a register sort, add the operand*/
        ...
        case 2:
            ...
            add_binary_line(binary_line,'c',1);/*Add the first line, with the sort number*/ 
            temp = (write_operand(type1,IC,first_operand));
            if(temp<0)/*Return error if can't write the operand*/
                return -1;
            IC += temp;
...
int write_operand(int type,int IC,char* first){
    ...
    /*Gets an operand, the IC, and the operand type, writes him in binary code, 
    if there any errors, returns them as negative value, if the operands are writen,
    returns the number of rows as positive number */

    char *temp_first = (char*)calloc(MAX_LABEL_NAME_SIZE, sizeof(char));
    char *temp_second = (char*)calloc(MAX_LABEL_NAME_SIZE, sizeof(char));
    int is_found;/*An indicator to the 'second_data_sort' function*/
    is_found = 0;
    switch(type){
...
int opearnd_type (char* name){
    ...
    /*Returns the given operand type:
    0 - Imidiate sort
    1 - Dyrect sort
    2 - Array sort
    3 - Register sort*/
...
int command_number(char* name){
    ...
    /*Looks for the name of the command in command list, returns his number
...
int command_line_check(char* line){
    ...
    /*Check the number of operands in the given line*/
...
char* command_code(char* name){
    ...
    /*Finds the command in commnad list, and returns the command binary code in string format
...
int operands_use_valid(int comm_num,int type1, int type2){
        ...
        /*Check the usage of the operands, with the command
        If valid returns 0, if not returns 1*/
...
void free_command_list() { 
    ...
    /*Clean the memmory alocated to command list */
